...
class UVLF_IllustrisTNG_ST_model1(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        minError = 0.2
        self.UVLF_IllustrisTNG = np.loadtxt(
            data.path["data"] + "/UVLF/UVLF_IllustrisTNG.txt"
        )
        self.UVLF_IllustrisTNG[:, 4] = np.array(
            list(
                map(
                    max,
                    zip(
                        minError * self.UVLF_IllustrisTNG[:, 3],
                        self.UVLF_IllustrisTNG[:, 4],
                    ),
                )
            )
        )
    ...
    def AUV(self, z, MUV):
        ...
        return max(
            0.0,
            4.43
            + 0.2 * np.log(10) * (1.99**2) * (sigmabeta**2)
            + 1.99 * self.betaAverage(z, MUV),
        )  # Meurer 1999
    ...
    def AP_effect(self, data):
        ...
        for z in self.zs:
            ...
            Vratio = (
                np.power(
                    self.rcomoving(
                        z + deltaz, self.Omega_m_IllustrisTNG, self.h_IllustrisTNG
                    ),
                    3,
                )
                - np.power(
            ...
            self.UVLF_data[self.UVLF_data[:, 0] == z, 1] = self.UVLF_data[
                ...
                / self.rcomoving(z, self.Omega_m_IllustrisTNG, self.h_IllustrisTNG)
            )
