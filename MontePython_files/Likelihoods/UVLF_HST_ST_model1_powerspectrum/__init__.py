...
class UVLF_HST_ST_model1_powerspectrum(Likelihood):
    ...
    def __init__(self, path, data, command_line):
        ...
        self.points_highres, self.weights_highres = np.polynomial.legendre.leggauss(
            1000
        )
        ...
        betadata = np.loadtxt(
            data.path["data"] + "/UVLF/Beta_parameters.txt", unpack=True
        )
    ...
    def calculate_sigma(self, R, kmin, kmax, pk, z, as2, as3, as4):
        ...
        window = lambda x: np.heaviside(1.0 - x * R, 1.0)
        return np.sqrt(
            self.integrator(lambda x: window(x) ** 2 * mps(x) * x**2, kmin, kmax, True)
            / 2.0
            / np.pi**2
        )
    ...
    def HMF_and_MUV_from_Mh(self, cosmo, data, z, Mh, ks):
        ...
        R = np.power(3.0 * Mh / (4.0 * np.pi * rhoM), 1.0 / 3) / self.cM
        sigma = self.calculate_sigma(
            R, min(ks), max(ks), cosmo.pk_cb_lin, z, As2, As3, As4
        )
        dsigmadM = (
            -cosmo.pk_cb_lin(1 / R, z)
            * self.scaling(1 / R, As2, As3, As4)
            / 12.0
            / np.pi**2
            / R**3
            / Mh
        ...
        sigma1 = self.calculate_sigma(
            np.power(3.0 * Mh / Q / (4.0 * np.pi * rhoM), 1.0 / 3) / self.cM,
            min(ks),
            max(ks),
            cosmo.pk_cb_lin,
            z,
        ...
        return (
            -self.AST
            * np.sqrt(2.0 * aST / np.pi)
            * (
                1.0
                + np.power(
                    np.power(sigma, 2) / (aST * np.power(self.deltaST, 2)), self.pST
                )
            )
            * (self.deltaST / sigma)
            * np.exp(-aST * np.power(self.deltaST, 2) / (2.0 * np.power(sigma, 2)))
            * (rhoM / (Mh * sigma))
            * dsigmadM,
            -2.5
            * np.log10(
