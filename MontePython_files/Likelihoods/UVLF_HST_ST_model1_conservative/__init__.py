...
class UVLF_HST_ST_model1_conservative(Likelihood):
    ...
    def MUV_from_Mh(self, cosmo, data, z, Mh):
        # conservative case for z-evolution of astro parameters
        alphastar, betastar, epsilonstar, Mc = (
            data.mcmc_parameters["alphastar_z%d" % z]["current"],
            data.mcmc_parameters["betastar_z%d" % z]["current"],
            10 ** data.mcmc_parameters["logepsilonstar_z%d" % z]["current"],
            10 ** data.mcmc_parameters["logMc_z%d" % z]["current"],
        )
