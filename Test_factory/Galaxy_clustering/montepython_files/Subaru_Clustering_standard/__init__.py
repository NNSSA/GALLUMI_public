...
class Subaru_Clustering_standard(Likelihood):
    ...
    def sigma_table(self, cosmo, data):
        ...
        Mhalos = np.geomspace(min(self.Mhalos)/3., max(self.Mhalos)*1.1, 100) # /3. because Mh/Q < Mh with Qmax = 2.5; *1.1 because of deltaMh
        zs = np.linspace(min(self.Subaru_redshifts), max(self.Subaru_redshifts), 10)
        sigmas = np.zeros((len(Mhalos), len(zs)))
        for numz, z in enumerate(zs):
            for numMh, Mh in enumerate(Mhalos):
                sigmas[numMh, numz] = cosmo.sigma_cb(np.power(3. * Mh / (4. * np.pi * rhoM), 1./3), z)
        return RegularGridInterpolator((Mhalos, zs), sigmas, bounds_error=False, fill_value=None)
    ...
    def dsigmadMh_interp(self, interpolator, z, Mh):
        ...
        return (sigma_plus_deltaMh - sigma_min_deltaMh) / (2. * deltaMh)
    ...
    def N_satellite(self, SHMFs, P_satellite, Msubhalos, Mmin_satellite):
        return simps(SHMFs * P_satellite * np.exp(-Mmin_satellite/Msubhalos), Msubhalos, axis=1)
    ...
    def astro_parameters(self, data, z):
        ...
        epsilonstar_central = 10**(data.mcmc_parameters['log10epsilonstar_central_slope']['current'] * np.log10((1 + z)/(1 + 5)) + data.mcmc_parameters['log10epsilonstar_central_icept']['current'])
        Mc_central = 10**(data.mcmc_parameters['log10Mc_central_slope']['current'] * np.log10((1 + z)/(1 + 5)) + data.mcmc_parameters['log10Mc_central_icept']['current'])
        ...
        epsilonstar_satellite = 10**(data.mcmc_parameters['log10epsilonstar_satellite_slope']['current'] * np.log10((1 + z)/(1 + 5)) + data.mcmc_parameters['log10epsilonstar_satellite_icept']['current'])
        Mc_satellite = 10**(data.mcmc_parameters['log10Mc_satellite_slope']['current'] * np.log10((1 + z)/(1 + 5)) + data.mcmc_parameters['log10Mc_satellite_icept']['current'])
        ...
        return alphastar_central, betastar_central, epsilonstar_central, Mc_central, sigma_MUV_central, Mmin_central, alphastar_satellite, betastar_satellite, epsilonstar_satellite, Mc_satellite, sigma_MUV_satellite, Mmin_satellite

    def MUV_from_Mh(self, cosmo, data, interpolator, z, Mh, alphastar, betastar, epsilonstar, Mc):
    ...
    def P_1_halo(self, HMFs, Mhalos, N_central, N_satellite, uNFW, ngals_of_z):
        return simps((2. * N_central * N_satellite * uNFW + N_satellite**2 * uNFW**2) * HMFs, Mhalos, axis=1) / ngals_of_z**2

    def P_2_halo(self, cosmo, k_array, z, HMFs, biases, Mhalos, N_central, N_satellite, uNFW, ngals_of_z):
        pk_lin = np.vectorize(lambda x: cosmo.pk_cb_lin(x, z))(k_array)
        return pk_lin * simps(HMFs * (N_central + N_satellite * uNFW) * biases, Mhalos)**2. / ngals_of_z**2
    ...
    def loglkl(self, cosmo, data):
        ...
        for numz, z in enumerate(self.Subaru_redshifts):
            ...
            for numzsub, zsub in enumerate(sub_redshifts):
                ...
                alphastar_central, betastar_central, epsilonstar_central, Mc_central, sigma_MUV_central, Mmin_central, alphastar_satellite, betastar_satellite, epsilonstar_satellite, Mc_satellite, sigma_MUV_satellite, Mmin_satellite = self.astro_parameters(data, zsub)

                rcomovings.append(self.rcomoving(zsub, Omega_m, hubble))
                drcomovingsdz.append(self.drcomovingdz(zsub, Omega_m, hubble))

                HMFs = self.HMF(data, sigma_interpolator, zsub, self.Mhalos)
                SHMFs = self.SHMF(zsub, self.Mhalos[:,np.newaxis], self.Msubhalos)

                MUV_avs_central = self.MUV_from_Mh(cosmo, data, sigma_interpolator, zsub, self.Mhalos, alphastar_central, betastar_central, epsilonstar_central, Mc_central)
                MUV_avs_satellite = self.MUV_from_Mh(cosmo, data, sigma_interpolator, zsub, self.Mhalos, alphastar_satellite, betastar_satellite, epsilonstar_satellite, Mc_satellite)

                halo_biases = self.halo_bias(sigma_interpolator, zsub, self.Mhalos)
                ...
                for num_MUV_bins in range(len(Subaru_data_at_z)):
                    ...
                    uNFW = self.fourier_of_NFW(data, zsub, self.Mhalos, self.k_array[:,np.newaxis])
                    Pgal[num_MUV_bins, numzsub, :] = self.P_1_halo(HMFs, self.Mhalos, N_c, N_s, uNFW, ngals_of_z) + self.P_2_halo(cosmo, self.k_array, zsub, HMFs, halo_biases, self.Mhalos, N_c, N_s, uNFW, ngals_of_z)
