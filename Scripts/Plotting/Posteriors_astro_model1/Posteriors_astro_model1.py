...
def plot_hist2d(datax, datay, ax, num_bins=30, weights=[None], color=None, zorder=0):
    ...
    ax.contour(interp_x_centers, interp_y_centers, np.transpose(interp_smoothed_hist), colors=[color, adjust_lightness(color,0.8)], linewidths=1.5, levels=ctr_level2d(interp_smoothed_hist.copy(), [0.68, 0.95]), zorder=zorder)

def corner_plot(data, names, num_bins=30, weights=[None], lims=[None], color=None, labelsize=None, ticksize=None, zorder=None, fig=None, axes=None):
    if not any(weights):
        weights = np.ones(len(data[:,0]))
    if not any(lims):
        lims = [[data[:, pos].min(), data[:, pos].max()] for pos in range(len(data[0,:]))]
    if color is None:
        color="black"
...
names = [r'$\sigma_8$', r'$\alpha_*$', r'$\beta_*$', r'$\epsilon_*^\mathrm{s}$', r'$\epsilon_*^\mathrm{i}$', r'$M_c^\mathrm{s}$', r'$M_c^\mathrm{i}$']
lims = [[0.4, 1.2], [-1.1, -0.2], [0., 3.], [-3., 1.], [-2.8, -1.2], [-3., 3.], [10.5, 12.7]]

fig, axes = corner_plot(data=chains_HST[:,[2,6,7,8,9,10,11]], names=names, num_bins=20, weights=chains_HST[:,0], lims=lims, color=colors[3], labelsize=22, ticksize=None, zorder=1, fig=None, axes=None)
corner_plot(data=chains_IllustrisTNG[:,[2,6,7,8,9,10,11]], names=names, num_bins=20, weights=chains_IllustrisTNG[:,0], lims=lims, color=colors[-1], labelsize=22, ticksize=None, zorder=0, fig=fig, axes=axes)

patch_blue = mpatches.Patch(color=colors[3], lw=1.5, label=r"$\mathrm{HST}$", alpha=0.8)
patch_yellow = mpatches.Patch(color=colors[-1], lw=1.5, label=r"$\mathrm{IllustrisTNG}$", alpha=0.8)
plt.legend(handles=[patch_blue, patch_yellow], loc="upper right", bbox_to_anchor=(-0.3, 6.2), prop={'size': 23})
...
for num in range(7): 
    ...
    axes[str(num)+str(0)].xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(4))
...
for num in range(5):
    ...
    axes[str(num)+str(1)].xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(3))
...
for num in range(4):
    ...
    axes[str(num)+str(2)].xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(4))
for num in range(2):
    axes[str(2)+str(num)].set_yticks([0.4, 1.2, 2.0, 2.8])
    axes[str(2)+str(num)].yaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(4))
...
for num in range(3):
    ...
    axes[str(num)+str(3)].xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
for num in range(3):
    axes[str(3)+str(num)].set_yticks([-2.5, -1.5, -0.5, 0.5])
    axes[str(3)+str(num)].yaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
...
for num in range(2):
    ...
    axes[str(num)+str(4)].xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
for num in range(4):
    axes[str(4)+str(num)].set_yticks([-2.5, -2.0, -1.5])
    axes[str(4)+str(num)].yaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))

axes[str(5)+str(0)].set_yticks([-2., 0., 2.])
...
axes[str(6)+str(5)].set_xticks([-2., 0., 2.])
...
for num in range(1):
    num += 5
    axes[str(num)+str(5)].set_xticks([-2., 0., 2.])
    axes[str(num)+str(5)].xaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(4))
for num in range(5):
    axes[str(5)+str(num)].set_yticks([-2., 0., 2.])
    axes[str(5)+str(num)].yaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(4))
...
for num in range(6):
    ...
    axes[str(6)+str(num)].yaxis.set_minor_locator(matplotlib.ticker.AutoMinorLocator(5))
