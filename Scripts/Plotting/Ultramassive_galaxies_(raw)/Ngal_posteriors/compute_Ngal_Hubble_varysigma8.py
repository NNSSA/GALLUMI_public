...
def Ngals(args):
    ...
    weight, lkl, As, alpha, beta, eps1, eps2, mc1, mc2, s1, QQ, amp, mean, sigma8, Omegam, sigma, epsilon = args

    cosmo = Class()
    args = {'P_k_max_1/Mpc': 20., 'T_ncdm': 0.71611, 'N_ur': 2.0308, 'N_ncdm': 1, 'tau_reio': 0.0544, 'n_s': 0.9649 , 'k_pivot': 0.05, 'omega_b': 0.02236, 'm_ncdm': 0.06, 'h': 0.6727, 'z_max_pk': 10.0, 'output': 'mPk, dTk', 'omega_cdm': 0.1202, 'ln10^{10}A_s': As}
    cosmo.set(args)
    ...
    for z, log10Mstar, Vol in JWST_Mstar:
        ...
        M_lower = 10**(log10Mstar) / 0.157 / epsilon # epsilon = 0.5
...
if __name__ == '__main__':
    ...
    data_HST = []
    for filepath in glob.iglob('../../Chains/Final_Gaussian_2_sigma8vary/*__*.txt'):
    ...
    with closing(Pool(processes=None)) as pool:
        for numm, Ngal_epsilon1 in enumerate(pool.imap(Ngals, list(np.column_stack((data_HST, np.repeat(0.1, len(data_HST[:,0])), np.repeat(0.3, len(data_HST[:,0]))))))):
            Ngals_HST.append(Ngal_epsilon1)
            if numm % 100 == 0:
                np.savetxt("Final_Ngals_HST_z6to10_epsilon0p3_varysigma8.txt", np.vstack(np.array(Ngals_HST)))
        pool.terminate()

    # for amp, mean in data_HST:
    #     Ngal_epsilon1.append(Ngals(cosmo, amp, mean, 0.1, 1.))

    np.savetxt("Final_Ngals_HST_z6to10_epsilon0p3_varysigma8.txt", Ngals_HST)