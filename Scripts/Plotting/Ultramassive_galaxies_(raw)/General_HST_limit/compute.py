...
def Ngals(args):
    ...
    Ngal = simps(HMF_interp(Mh_array_integrate), Mh_array_integrate)
...
if __name__ == '__main__':
    ...
    Mcut_array = np.geomspace(1e9, 5e13, 40)

    Ngals_HST_full = []
    for numm, Mcut in enumerate(Mcut_array):
        Ngals_HST = []
        weights_HST = []
        Ngals_HST.append(Mcut)
        weights_HST.append(Mcut)
        with closing(Pool(processes=None)) as pool:
            for weight, Ngal_from_Hubble in pool.imap(Ngals, list(np.column_stack((data_HST, np.repeat(0.1, len(data_HST[:,0])), np.repeat(Mcut, len(data_HST[:,0])))))):
                Ngals_HST.append(Ngal_from_Hubble)
                weights_HST.append(weight)
            pool.terminate()
        if numm == 0:
            Ngals_HST_full.append(np.vstack((np.hstack(np.array(weights_HST)), np.hstack(np.array(Ngals_HST)))))
        else:
            Ngals_HST_full.append(np.hstack(np.array(Ngals_HST)))
        if numm % 2 == 0:
            np.savetxt("Ngals_HST_z6to10_z7p5forecast.txt", np.vstack(np.array(Ngals_HST_full)))
